...
# Generate fastText embeddings for all descriptions

def feature_model(X_train, X_test, max_n_gram, embedding_sz):

    global ft_model

    train_sentences = X_train.apply(lambda x: x.split())

    ft_model = FastText(train_sentences, size=embedding_sz, window=5, min_count=5, workers=1, sg=1, max_n=max_n_gram, min_n=2, seed=42)
...
# Maximum number of embedding is 300
embedding_sz = 300
max_n_gram = 6

print("Current embedding size:", embedding_sz)
print("Current max_n_gram:", max_n_gram)

t_time = time.clock()
X_train_transformed, X_test_transformed = feature_model(X_train, X_test, max_n_gram, embedding_sz)
print("Building vocab:", time.clock() - t_time)

# Iterate over each label and corresponding optimal classifier for testing
